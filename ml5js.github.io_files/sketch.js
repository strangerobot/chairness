...
const classifier = ml5.imageClassifier('Darknet', () => { result.innerText = "Chair";});
...
function classifyImage() {
  classifier.predict(image,999, (err, results) => {

    var totalprob = 0;
    //console.log(i + "  " + results[i].className + "  " + results[i].probability);
    for (var i = 0; i < 999; i++) {
      if (results[i].className.localeCompare("barber chair") == 0 || results[i].className.localeCompare("folding chair") == 0 || results[i].className.localeCompare("rocking chair, rocker") == 0 || results[i].className.localeCompare("toilet seat") == 0) {
        console.log(i + "  " + results[i].className + "  " + results[i].probability);
        totalprob += results[i].probability;
      }
    }
    result.innerText = "Chair";
    let prob = 100 * totalprob;
...
// for(var i=0;i<999;i++)
// {
// if(results[i].localeCompare("barber chair")==0 || results[i].localeCompare("folding chair")==0 || results[i].localeCompare("rocking chair, rocker")==0 )
// {
//   console.log(i+"  "+results[i].className+"  "+results[i].probability);
//   totalprob+=results[i].probability;
// }
